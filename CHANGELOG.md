...
- **Added: one-edit typo tolerance.** The `transposed` tier is joined by three more, each naming the correction the *query* needed: `inserted` (one character too many, "generric"), `deleted` (one missing, "genric"), `substituted` (one wrong, "genaric"). Measured as MRR of the intended item over the ascii corpus, before → after: insertion **0.004 → 0.998**, substitution **0.000 → 0.996**; deletion already matched via the fuzzy tier and now ranks as a typo (0.968 → 0.962 at the new penalty). Long-text junk rate stays at **0%**.
...
- **Breaking:** the `Tier` union gains four members (`transposed`, `inserted`, `deleted`, `substituted`). A TypeScript consumer with an exhaustive `switch` over `Tier` stops compiling until the new arms are added; `score <= SCORES.CONTAINS` still separates literal matches from speculative ones without naming any tier.
