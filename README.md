...
- **0.3 ms** per query over 10k items, ~2.2 ms over 100k, optimised for search-as-you-type
...
- **Search-as-you-type**: each keystroke rescans only the previous one's survivors, so the first keystroke is the expensive one and the rest cost ~40% less, even over 100k items.
...
Speed is not the constraint at any realistic size; a prebuilt Krino index answers a 100,000-item query in ~2.2 ms (0.3 ms at 10k), and `fuzzyMatch` over a 16,000-character document costs 0.28 ms.
...
- **Speed** (per-query mean): ~0.3 ms over 10k items and ~2.2–3.6 ms over 100k, ~5–6× faster than its parent microfuzz; uFuzzy edges Krino on pure-ascii 100k corpora, while Krino leads accented data outright.
...
It is not the fastest engine in the set: `uFuzzy` is ~1.6× quicker per query at 100k on ascii and keeps no index, trading most of the match quality to get there.
Two workloads genuinely point elsewhere:

- **Typos beyond a single edit must still match** (user-typed queries over messy data): the four typo tiers cover every one-character mistake, but two or more edits in one query need real edit distance. Pick `Fuse.js` (Bitap) or `fast-fuzzy`, at 3–4× the bundle, ~6–18 ms queries, and ~90–450-row result sets.
- **Raw query throughput at 100k+, bare index output is enough**: `uFuzzy` is ~1.6× faster per query on ascii, and keeps no index at all. On accented data the order reverses and Krino leads. It costs roughly half Krino's match quality (0.38 MRR to 0.77 on ascii), so this is a genuine trade rather than a free win.
...
(Already on `@nozbe/microfuzz`? Krino is its rebuild: same subsequence approach plus tier, ESM, and 5–6× faster. See [MIGRATION.md](./MIGRATION.md).)
