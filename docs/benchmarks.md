...
Every table below is generated, and `pnpm bench` regenerates all of them: it measures, writes [`bench/results.json`](../bench/results.json), rewrites each table in place, and redraws the Pareto charts.
No cell in this document is typed by hand.
...
First, Krino's per-keystroke cost falls _1.7×_ between the first keystroke and the rest (4.40 → 2.49) as the survivor cache narrows: the 3-character query is the widest candidate set the session ever sees, and it is the one that pays.
Second, uFuzzy's flat ~2.6 ms takes this short session's total (7.72 vs 9.54), and by the completed word the two are inside the tie band (2.65 vs 2.60): its bare-index-array output owns the short prefixes, at 0.42 MRR to Krino's 0.80.
Third, microfuzz stays flat at ~26–33 ms: same subsequence approach with no survivor cache, so nothing narrows between keystrokes.
