...
The distortion is visible across runs, not just within one: Krino's 100k build usually ranges from ~18 to ~51 ms on the same machine depending on load, while its standalone floor (best-of-N, GC quiet) is ~13–20 ms; this document's run measured 49 ms.
...
1. **Every query is derived from the corpus.**
   ...
   Deriving from a real item is also what makes _rank_ measurable at all: each query has a known right answer to look for.
...
|-------|---------:|-----------------:|-----------:|--------:|-----------------:|
| 10k   |  2.86 ms |         11.53 ms |   38.91 ms | 1.14 ms |         10.61 ms |
| 100k  | 49.37 ms |         95.35 ms |  588.41 ms | 8.95 ms |         99.41 ms |
...
Fuse.js's near-free build is the flip side of its slow queries: its "index" is trivial and the work is deferred to query time (1.14 ms at 10k and 8.95 at 100k, the cheapest build in the set at both sizes).
...
Krino prepares eagerly, so a 100k list swap costs ~49 ms once (this run's cell); keystrokes then ride the prefix cache (see the session table at the bottom).
...
A second plain-word probe from elsewhere in the corpus; the result is the same, Krino ranks the source first with the smallest set.
...
These are the same two words in the opposite order, which is what makes this the probe that actually isolates tokenized matching.
...
This is the cleanest split in the document. A subsequence engine cannot represent an _extra_ character at all (there is no way to skip a query character), so microfuzz, fuzzysort and uFuzzy return _0_ matches rather than ranking the source poorly.
...
Krino answers in _1%_ of a matching query because the character-class mask rejects on one integer AND, before any regex runs; every other engine pays 56–78%, because a miss is only knowable after the full scan.
This is the probe the gate architecture exists for, and the place its cost model is most visible in isolation.
...
Result-set size is deliberately _not_ a scorecard column: in a ranked UI any result list slices to the top N, so a large return costs a picker nothing (see "The corpus and the fifteen probes").
...
_Krino (acronym) tops both corpora outright_ (0.84 mixed / 0.77 ascii): only a deliberate acronym tier ranks initials first, while Fuse.js _loses the source_ on that query and lands at 0.75, arriving with ~90-row median lists (mean ~185) at ~17 ms where Krino's answer costs 0.18 ms.
On structured queries Krino returns a median of _7_ rows where Fuse ships ~90; a picker slices to the top rows either way, a filter-style UI shows all of them.
...
<picture>
  ...
  <img alt="Mixed-corpus accuracy (MRR) vs. query ms with indexes prebuilt, log scale, as a Pareto frontier. The frontier is Krino (acronym) at 0.84 MRR and 0.18 ms, with base Krino close behind at 0.80; every other configuration, including Fuse.js at 0.75 and 17 ms, is dominated." src="./pareto-query-light.svg">
</picture>
...
It does not own the whole curve. fuzzysort's 0.15 ms query is cheaper than Krino's 0.18 ms, so anyone willing to trade 0.30 MRR for 0.03 ms has a point on the frontier.
...
uFuzzy (all opts) is a genuine frontier point here, at a 0.88 ms total against Krino's 1.89: its typo mode costs it almost nothing per query (0.26 ms) and triples its MRR over its defaults, and the 0.62 ms of that total is latinizing the haystack, preparation it must be charged for, since it is the configuration competing on this axis.
...
The acronym configuration runs strictly _more_ code per query (an extra tier, plus the one-edit rescues on candidates that reach it); its 147% cell is that price plus load swing.
...
Krino beats its parent `@nozbe/microfuzz` on both corpora (~4.8× on ascii and ~6.2× on mixed, base configs).
The (all opts) rows stay cheap in absolute terms across the board.
_uFuzzy is the faster engine at 100k on ascii_: 2.39 ms to Krino's 4.08 (59%). On mixed with both folding the order reverses, Krino's 2.40 ms to uFuzzy's 2.85 (119%).
...
That capability gap is the trade: uFuzzy's typo configuration costs it almost nothing here (2.73 ms on ascii) but ranks the queried item at 0.38 MRR against Krino's 0.77.
...
**fast-fuzzy is corpus-sensitive**: its trie shines on shared-prefix data but this natural-language corpus prunes less, dropping it among the slowest (on a combinatorial word-grid it was ~4× _faster_ than Krino; corpus shape moves these numbers a lot).
For 100k+ corpora where raw query throughput dominates and a bare index array is enough output, uFuzzy is the faster choice on ascii, where Krino's case rests on rank quality and richer output rather than speed. On mixed with both folding that reverses and Krino leads on speed too.
...
First, Krino's per-keystroke cost falls _3.6×_ across the word (7.17 → 2.01) as the survivor cache narrows, and the first keystroke is the expensive one, because a 3-character query is the widest candidate set the session ever sees.
...
- **Raw throughput at 100k+, one query at a time.** uFuzzy is ~1.7× faster per query on ascii (2.39 vs 4.08 ms), and it keeps no index, so its cold one-shot is a small fraction of Krino's ("Reading the speed numbers"). On mixed with both folding it is Krino that leads (2.40 vs 2.85 ms), so this carve-out is an ascii-corpus one.
  ...
  Typing still favours Krino outright: the session probe shows the prefix cache dropping keystrokes below uFuzzy's flat ~2.7 ms by the end of a word (2.01 vs 2.72 on `grady`).
...
- **fast-fuzzy**: the heaviest build (~590 ms at 100k) and slowest queries on these corpora; its trie rewards shared-prefix data, which natural-language corpora don't provide.
